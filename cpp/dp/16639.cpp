    ...
    for (int cnt = 1; cnt < numberCnt; cnt++) {
        for (int idx = 0; idx < numberCnt - cnt; idx++) {
            for (int i = 1, j = cnt; i <= cnt; i++, j--) {
                ...
                char op = s[opIdx];
                int candidates[4] = { calculate(maxCache[idx][idx + cnt - j], maxCache[idx + i][idx + cnt], op), calculate(maxCache[idx][idx + cnt - j], minCache[idx + i][idx + cnt], op), calculate(minCache[idx][idx + cnt - j], maxCache[idx + i][idx + cnt], op), calculate(minCache[idx][idx + cnt - j], minCache[idx + i][idx + cnt], op) };
